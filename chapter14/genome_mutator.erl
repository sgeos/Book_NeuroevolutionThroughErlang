...
-export(
  [
    ...
    test/2,
    mutate/1,
    mutate_SearchParameters/1,
    apply_ESMutators/2,
    apply_Mutators/2,
    apply_NeuralMutator/1,
    select_random_MO/1,
    select_random_MO/3,
    mutate_tuning_selection/1,
    mutate_tuning_duration/1,
    ...
    add_outlink/1,
    filter_OutlinkIdPool/3,
    add_inlink/1,
    filter_InlinkIdPool/3,
    add_neuron/1,
    outsplice/1,
    get_NewLI/4,
    get_NextLI/3,
...
-define(
  ...
  [
    ...
    mutate_tuning_duration, % !!! does not appear to exist in the official repository
...
% This is a simple function that executes the test() function the number of times with which the
% long_test/1 function was initially called. The test/0 function executes mutate( test ), which
% applies a random number of mutation operators to the genotype, where that number ranges from 1 to
% sqrt( Tot_neurons ). After all the mutation operators have been applied succesfully, the function
% executes exoself:start( test, void ), mapping the genotype to phenotype, to test whether the
% resulting NN system is functional.
long_test( TotMutations ) when 0 < TotMutations ->
...
%
% The function mutate/1 first updates the generation of the agent to be mutated, then calculates the
% number of mutation operators to be applied to it by executing the
% tot_topological_mutations:TTM_Name/2 function, and then finally runs the apply_Mutators/2 function,
% which mutates the agent. Once the agent is mutated, the function updates its fingerprint by
% executing genotype:update_finrgerprint/1.
mutate( Agent_Id ) ->
...
% mutate_SearchParamters/1 with a probability of ?SEARCH_PARAMETERS_MUTATION_PROBABILITY applies a
% random number between 1 and length(?ES_MUTATORS) of evolutioanry strategy mutation operators from
% the ?ES_MUTATORS list.
mutate_SearchParameters( Agent_Id ) ->
...
% apply_ESMutators/2 with uniform distribution chooses a random evolutionary strategy mutation
% operator from the ?ES_MUTATORS list of such functions, and applies it to the agent. Whether the
% mutation succcessful or not, the function counts down the total number of mutation operators left
% to apply. This is to ensure that if the researcher set for each such evolutionary strategy to be
% static, having only one available mutatable parameter for every agent, the system will eventually
% try to mutate the strategy TotMutations number of times, and then return to the caller.
...
% unsuccessful mutation operator was never applied. The mutation operator to be applied to the agent
% is chosen randomly from the agent's mutation_operators list.
...
% select_random_MO/1, using the analogy of a roulette wheel, first calculates the entire are of the
% wheel by summing together all the slice sizes of the parts. The function than chooses randomly a
% spot on the wheel, and through select_random_MO/3 calculates where that spot is located, with
% regards to the mutation operator that it falls on. Since some slices are larger then others, they
% will have uniformly larger probabilities of being selected.
...
select_random_MO( [ { MO, SliceSize } | MOs ], Choice, Range_From ) ->
  ...
  case ( Range_From =< Choice ) and ( Choice =< Range_To ) of
    true ->
      MO;
    false ->
...
% mutate_tuning_selection/1 function checks if there are any other than the currently used tuning
% selection functions available in the agent's constraint. If there is, then it chooses a random one
% from this list, and sets the agent's tuning_selection_f to it. If there are no other tuning
% selection functions, then it exits with an error.
mutate_tuning_selection( Agent_Id ) ->
  A = genotype:read( { agent, Agent_Id } ),
  case ( A#agent.constraint )#constraint.tuning_selection_fs -- [ A#agent.tuning_selection_f ] of
    [] ->
      exit(
        "--- --- --- Error: mutate_tuning_selection/1: "
        "Nothing to mutate, only a single function available."
      );
...
% !!! mutate_tuning_duration/1 is not defined in the official repository.
mutate_tuning_duration( _Agent_Id ) ->
  stub.

% !!! mutate_annealing_parameter/1 -> mutate_tuning_annealing/1
%
% mutate_tuning_annealing/1 function checks if there are any other than the currently used tuning
% annealing parameters available in the agent's constraint. If there is, then it chooses a random one
% from this list, and sets the agent's annealing_parameter to it. If there are no other tuning
% annealing parameters, then it exits with an error.
mutate_tuning_annealing( Agent_Id ) ->
  A = genotype:read( { agent, Agent_Id } ),
  case ( A#agent.constraint )#constraint.annealing_parameters -- [ A#agent.annealing_parameter ] of
    [] ->
      exit(
        "--- --- --- Error: mutate_tuning_annealing/1: "
        "Nothing to mutate, only a single function available."
      );
...
% !!! Function body was a copy of mutate_tuning_selection/1 in the official repository.
%
% mutate_tot_topological_mutations/1 function checks if there are any other than the currently used
% tuning tot topological mutation functions available in the agent's constraint. If there is, then it
% chooses a random one from this list, and sets the agent's tot_topological_mutations_f to it. If
% there are no other functions that can calculate tot topological mutations, then it exits with an
% error.
mutate_tot_topological_mutations( Agent_Id ) ->
  A = genotype:read( { agent, Agent_Id } ),
  case
    ( A#agent.constraint )#constraint.tot_topological_mutations_fs --
      [ A#agent.tot_topological_mutations_f ]
  of
    [] ->
      exit(
        "--- --- --- Error: mutate_tot_topological_mutations/1: "
        "Nothing to mutate, only a single function available."
      );
...
% The mutate_pf/1 function chooses a random neuron, and then changes its currently used plasticity
% function into another one available from the neural_pfs list of the agent's constraint record.
mutate_pf( Agent_Id ) ->
  A = genotype:read( { agent, Agent_Id } ),
  Cx_Id = A#agent.cx_id,
  Cx = genotype:read( { cortex, Cx_Id } ),
...
% The mutate_aggrf/1 function chooses a random neuron, and then changes its currently used
% aggregation function into another one available from the neural_aggr_fs list of the agent's
% constraint record.
...
link_FromNeuron( FromN, ToId, Generation ) ->
  ...
  case lists:member( ToId, FromOutput_Ids ) of
    true ->
      exit(
        "--- --- --- Error: link_FromNeuron: Can not add ToId to neuron.  "
...
link_ToNeuron( FromId, FromOVL, ToN, Generation ) ->
  ...
  case lists:keymember( FromId, 1, ToInput_IdPs ) of
    true ->
      exit(
        "--- --- --- Error: link_ToNeuron: Can not add FromId to neuron.  "
        "~p is already a member of ~p~n",
...
link_FromSensor( FromS, ToId, Generation ) ->
  ...
  case lists:member( ToId, FromFanout_Ids ) of
    ...
    false ->
      FromS#sensor{ fanout_ids=[ ToId | FromFanout_Ids ], generation=Generation }
  end.
...
cutlink_FromNeuron( FromN, ToId, Generation ) ->
  ...
  case lists:member( ToId, FromOutput_Ids ) of
    false ->
      exit(
        "--- --- --- Error: cutlink_FromNeuron: Can not remove ToId.  "
...
cutlink_ToNeuron( FromId, ToN, Generation ) ->
  ...
  case lists:keymember( FromId, 1, ToInput_IdPs ) of
    false ->
      exit(
        "--- --- --- Error: cutlink_ToNeuron: Can not remove FromId.  "
        "~p is not a member of ~p~n",
        [ FromId, ToN#neuron.id ]
      );
    true ->
...
% The function filter_OutlinkIdPool/3 uses the connection_architecture specification in the
% constraint record of the agent to return a filtered neuron id pool. For the feedforward
% connection_architecture, the function ensures that only the neurons in the forward facing layers
% are allowed in the id pool.
filter_OutlinkIdPool( C, N_Id, N_Ids ) ->
  case C#constraint.connection_architecture of
    ...
    feedforward ->
      { { LI, _ }, neuron } = N_Id,
      [
        { { Outlink_LI, Outlink_UniqueId }, neuron }
          || { { Outlink_LI, Outlink_UniqueId }, neuron } <- N_Ids, LI < Outlink_LI
      ]
  end.
...
filter_InlinkIdPool( C, N_Id, N_Ids ) ->
  case C#constraint.connection_architecture of
    ...
    feedforward ->
      { { LI, _ }, neuron } = N_Id,
      [
        { { Inlink_LI, Inlink_UniqueId }, neuron }
          || { { Inlink_LI, Inlink_UniqueId }, neuron } <- N_Ids, Inlink_LI < LI
      ]
  end.
  ...
  case ( FromElementId_Pool == [] ) or ( Outlink_NIdPool == [] ) of
    true ->
      exit(
        ...
        "Inlink_NIdPool or Outlink_NIdPool is empty."
      );
    false ->
      From_ElementId =
        lists:nth( rand:uniform( length( FromElementId_Pool ) ), FromElementId_Pool ),
...
add_actuatorLink( Agent_Id ) ->
  ...
  case N_Ids -- A#actuator.fanin_ids of
    [] ->
      exit( "--- --- --- Error: add_actuatorLink: Actuator already connected from all Ids." );
    Available_Ids ->
      N_Id = lists:nth( rand:uniform( length( Available_Ids ) ), Available_Ids ),
...
add_sensor( Agent_Id ) ->
  ...
  case
    morphology:get_Sensors( Morphology ) --
      [
        ( genotype:read( { sensor, S_Id } ) )#sensor{
          id=undefined,
          cx_id=undefined,
          fanout_ids=[],
