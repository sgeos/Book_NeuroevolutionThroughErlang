...
construct_Cortex( Agent_Id, Generation, SpecCon ) ->
  ...
  Sensors = [
    S#sensor{ id={ { -1, generate_UniqueId() }, sensor }, cx_id=Cx_Id }
    || S <- morphology:get_InitSensors( Morphology )
  ],
  Actuators = [
    A#actuator{ id={ { +1, generate_UniqueId() }, actuator }, cx_id=Cx_Id }
...
update_fingerprint( Agent_Id ) ->
  ...
  GeneralizedSensors = [
    ( read( { sensor, S_Id } ) )#sensor{ id=undefined, cx_id=undefined }
    || S_Id <- Cx#cortex.sensor_ids
  ],
  GeneralizedActuators = [
    ( read( { actuator, A_Id } ) )#actuator{ id=undefined, cx_id=undefined }
