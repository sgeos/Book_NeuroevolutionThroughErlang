...
% highest fitness instead of their current state which yielded the current lower fitness score. If
% on the other hand the new fitness is higher than the previously highest achieved fitness, then the
...
% NN's fitness a maximum S#state.max_attempts number of times. If that is the case, the exoself
% process backs up the updated NN (the updated, tuned weights) to database using the
% backup_genotype/2 function, prints to screen that it is terminating, and sends to the
% population_monitor the acumulated statistics (highest fitness, evaluation count, cycle count...).
% On the other hand, if the exoself is not yet done tuning the neural weights, it has not yet reached
% its ending condition, it uses a tuning_selection_function to compose a list of tuples:
% [{NId,Spread}...] of neuron ids and the perturbation spread values, where the spread is the range
% from which the perturbation is randomly chosen. The spread itself is based on the age of the
% selected neuron, using the annealing_factor value, which when set to 1 implies that there is no
% annealing, and when set to a value less than 1, decreases the Spread. Once this list of elements is
% composed, the exoself sends each of the neurons a message to perturb their synaptic weights using
...
loop( S) ->
  receive
    { Cx_PId, evaluation_completed, Fitness, Cycles, Time } ->
      ...
      io:format( "U_HighestFitness: ~p  U_Attempt: ~p~n", [ U_HighestFitness, U_Attempt ] ),
      ...
      case S#state.max_attempts =< U_Attempt of
        true -> % end training
          ...
          gen_server:cast(
            S#state.pm_pid,
            { S#state.agent_id, terminated, U_HighestFitness, U_EvalAcc, U_CycleAcc, U_TimeAcc }
          );
        false -> % continue training
          ...
          [
            ets:lookup_element( IdsNPIds, NId, 2 ) ! { self(), weight_perturb, Spread }
            || { NId, Spread } <- ChosenNIdPs
          ],
