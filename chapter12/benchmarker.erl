...
-export(
  [
    ...
    gen_plot/1,
    gen_plot/4,
    genplot/1, % !!! gen_plot/1 and genplot/1 - not good to have functions with such similar names
    genplot/2,
    print_plot/2,
    trace2graph/1
  ]
...
% for information about each run.      
start( Id ) ->
  PMP = #pmp{
    op_mode = gt,
    population_id = test,
...
prep_Traces( [ T | Traces ], Morphology, Acc ) ->
  Morphology_Trace = lists:flatten(
    [
      [ S || S <- Stats, S#stat.morphology == Morphology ]
      || Stats <- T#trace.stats
    ]
  ),
...
write_Graphs( [ G | Graphs ], Graph_Postfix ) ->
  ...
  U_G = G#graph{
    evaluation_Index = [
      500 * Index
      || Index <- lists:seq( 1, length( G#graph.avg_fitness ) )
    ]
  },
  { ok, File } = file:open(
    ?DIR ++ "graph_" ++ atom_to_list( Morphology ) ++ "_" ++ Graph_Postfix,
    write
  ),
  io:format( File, "#Avg Fitness Vs Evaluations, Morphology: ~p~n", [ Morphology ] ),
  lists:foreach(
    fun( { X, Y, Std } ) ->
      io:format( File, "~p ~p ~p~n", [ X, Y, Std ] )
    end,
  ...
  io:format(
    File,
    "~n~n#Specie-Population Turnover Vs Evaluations, Morphology: ~p~n",
    [ Morphology ]
  ),
  lists:foreach(
    fun( { X, Y } ) ->
