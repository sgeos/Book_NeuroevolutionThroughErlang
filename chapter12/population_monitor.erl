...
-export(
  [
    ...
    % gen_server callbacks
      ...
      extract_AgentIds/2,
      delete_population/1,
    % !!! unused functions, adding to exports
      calculate_PopulationFitness/5,
      calculate_PopulationDiversity/3,
      print_SpecieDiversity/1
  ]
).
-behavior( gen_server ).
-include( "records.hrl" ).
...
handle_cast(
  ...
  case ( Evaluation_Limit =< S#state.tot_evaluations ) or ( Fitness_Goal < NewBestFitness ) of
    ...
    false ->
      ...
      Valid_AgentSummaries = case
        S#state.specie_size_limit =< length( ProperlySorted_AgentSummaries )
      of
        true ->
      ...
      Top_AgentSummaries = lists:sublist(
        U_DeadPool_AgentSummaries,
        round( S#state.specie_size_limit * S#state.survival_percentage )
      ),
...
handle_cast( { _From, evaluations, Specie_Id, AgentEvalAcc, AgentCycleAcc, AgentTimeAcc }, S ) ->
  ...
  U_S = case S#state.step_size =< U_EvalAcc of
    true ->
...
terminate( Reason, S ) ->
  case S of
    ...
    _ ->
      ...
      io:format(
        "--- --- --- Tot Agents: ~p  Population Generation: ~p  Tot_Evals: ~p~n",
        [
          S#state.tot_agents,
          S#state.pop_gen,
          S#state.tot_evaluations
        ]
      ),
...
init_population( Init_State, Specie_Constraints ) ->
  ...
  _OpMode = Init_State#state.op_mode, % !!! Why is OpMode bound?  Underscore added.
  F = fun() ->
...
calculate_PopulationFitness(
  ...
  calculate_PopulationFitness(
    Population_Id,
    Specie_Ids,
    [ { Specie_Id, AvgFitness } | AvgFAcc ],
    [ { Specie_Id, MaxF } | MaxFAcc ],
    [ { Specie_Id, MinF } | MinFAcc ]
  );
...
gather_STATS( Population_Id, EvaluationsAcc ) ->
  ...
  F = fun() ->
    ...
    SpecieSTATS = [
      ...
      || Specie_Id <- P#population.specie_ids
    ],
...
calculate_SpecieDiversity( { specie, S } ) ->
  Agent_Ids = S#specie.agent_ids,
  _Diversity = calculate_diversity( Agent_Ids ); % !!! Why is Diversity bound?  Underscore added.
...
print_SpecieDiversity( [ Specie_Id | Specie_Ids ] ) ->
  ...
  Morphology = ( S#specie.constraint )#constraint.morphology,
  io:format(
    "Specie id: ~p~n  Specie morphology: ~p~n  Diversity: ~p~n",
    [ Specie_Id, Morphology, get( { diversity, Specie_Id } ) ]
  ),
