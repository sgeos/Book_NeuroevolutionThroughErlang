...
-define(
  INIT_CONSTRAINTS,
  [
    #constraint{ morphology=Morphology, connection_architecture=CA }
...
handle_cast(
  { Agent_Id, terminated, _Fitness, AgentEvalAcc, AgentCycleAcc, AgentTimeAcc }, S
) when S#state.selection_algorithm == competition ->
...
terminate( Reason, S ) ->
  case S of
    ...
    _ ->
      ...
      % },
      io:format(
        "--- --- ---~n  Tot Agents:~p~n  Population Generation:~p~n"
        "  Eval_Acc:~p~n  Cycle_Acc:~p~n  Time_Acc:~p~n",
        [
          S#state.tot_agents,
          S#state.pop_gen,
          S#state.eval_acc,
...
mutate_Specie( Specie_Id, PopulationLimit, NeuralEnergyCost, Selection_Algorithm ) ->
  ...
  case Selection_Algorithm of
    competition ->
      ...
      io:format( "Valid agent summaries: ~p~n", [ Valid_AgentSummaries ] ),
      io:format( "Invalid agent summaries: ~p~n", [ Invalid_AgentSummaries ] ),
  ...
  [ TopFitness | _ ] = FList,
  U_InnovationFactor = case S#specie.innovation_factor < TopFitness of
    true ->
      0;
    false ->
...
% The top3'3 function is part of a very simple selection algorithm, which just selects the top 3 most
% fit agents, and then uses the create_MutantAgentCopy/1 function to create their offspring.
